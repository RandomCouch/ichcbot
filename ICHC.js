...
module.exports = function(config){
    ...
    parseResponse = function(lines){
        lines.forEach(function(line){
            ...
            switch(eventType){
				...
				case 'mods':
					var parse = line.match(/^.+\|(.+?)\|(.+)/);
					console.log("MOD MESSAGE RECEIVED " + line);
					var username = parse[1];
					var message = htmlEntityDecode(parse[2]);
					if(username != config.user){
						trigger('mods', {
                            username: username,
                            message: message
    ...
    chain = {
		...
		mods: function(username, message, success, fail){
			return this.send("/tellmods " + message, success, fail);
		},
